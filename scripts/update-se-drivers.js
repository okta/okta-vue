...
axios.get(chromeDriverUrl).then((response) => {
  ...
  // execFileSync(`${__dirname}/../test/e2e/node_modules/.bin/webdriver-manager`, ["update",  "--versions.chrome", chromeDriverVersion, "--gecko", "false", "--versions.standalone", "latest"]);
  execFileSync(`${__dirname}/../node_modules/webdriver-manager/bin/webdriver-manager`, ["update",  "--versions.chrome", chromeDriverVersion, "--gecko", "false", "--versions.standalone", "latest"]);
